...
const PASSWORD_RESET_SECRET: Secret = (process.env.JWT_PASSWORD_RESET_SECRET ||
  'change_me_password_reset') as Secret;
const ACCESS_EXPIRES: SignOptions['expiresIn'] = (process.env.JWT_ACCESS_EXPIRES ||
  '15m') as SignOptions['expiresIn'];
const REFRESH_EXPIRES: SignOptions['expiresIn'] = (process.env.JWT_REFRESH_EXPIRES ||
  '7d') as SignOptions['expiresIn'];
const EMAIL_EXPIRES: SignOptions['expiresIn'] = (process.env.JWT_EMAIL_EXPIRES ||
  '24h') as SignOptions['expiresIn'];
const PASSWORD_RESET_EXPIRES: SignOptions['expiresIn'] = (process.env.JWT_PASSWORD_RESET_EXPIRES ||
  '1h') as SignOptions['expiresIn'];

if (!ACCESS_SECRET || !REFRESH_SECRET || !EMAIL_SECRET || !PASSWORD_RESET_SECRET) {
  console.warn(
    'One or more JWT secrets are not configured (access, refresh, email, password reset).'
  );
}
...
export function signEmailVerificationToken(userId: string) {
  return jwt.sign({ sub: userId, type: 'email_verification' } as EmailPayload, EMAIL_SECRET, {
    expiresIn: EMAIL_EXPIRES,
  });
}
...
export function signPasswordResetToken(userId: string) {
  return jwt.sign(
    { sub: userId, type: 'password_reset' } as PasswordResetPayload,
